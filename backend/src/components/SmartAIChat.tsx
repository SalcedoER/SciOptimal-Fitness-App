...
export default function SmartAIChat() {
  ...
  const simulateTyping = (response: any) => {
    ...
    setTimeout(() => {
      addMessage('ai', response.content, {
        ...
        data: response.data,
        modalities: response.modalities || ['text'],
        emotions: response.emotions || ['analytical'],
  ...
  return (
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      {/* AI Insights Header */}
      {aiInsights && (
        <Box sx={{ p: 2, borderBottom: '1px solid #333', background: 'rgba(0,0,0,0.2)' }}>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
            <Typography variant="h6" sx={{ color: '#ffffff !important', display: 'flex', alignItems: 'center' }}>
              <AutoAwesome sx={{ mr: 1, color: '#00e676' }} />
              Smart AI Coach
            </Typography>
            <Box sx={{ display: 'flex', gap: 1 }}>
              <Chip 
                icon={<Memory />} 
                label={`${aiInsights.memoryStats.totalMemories} memories`} 
                size="small" 
                sx={{ bgcolor: 'rgba(0,230,118,0.2)', color: '#00e676' }}
              />
              <Chip 
                icon={<Analytics />} 
                label={`${Math.round(aiInsights.adaptationInsights.successRate * 100)}% accuracy`} 
                size="small" 
                sx={{ bgcolor: 'rgba(33,150,243,0.2)', color: '#2196f3' }}
              />
            </Box>
          </Box>
          <Box sx={{ display: 'flex', gap: 2, fontSize: '0.8rem', color: '#ccc' }}>
            <span>Learning Rate: {Math.round(aiInsights.memoryStats.learningRate * 100)}%</span>
            <span>Predictions: {aiInsights.predictionInsights.predictions}</span>
            <span>Progress: {Math.round(aiInsights.goalInsights.progress * 100)}%</span>
          </Box>
        </Box>
      )}
                    ...
                    <Box sx={{ mt: 1, display: 'flex', gap: 1, flexWrap: 'wrap' }}>
                      ...
                      {message.modalities && (
                        <Chip 
                          ...
                          size="small" 
                          sx={{ bgcolor: 'rgba(255,152,0,0.2)', color: '#ff9800', fontSize: '0.7rem' }}
                        />
                      )}
                    </Box>
                  )}
